...
class CentralSpinExperiment ():
	...
	def generate (self, nr_spins, concentration = .1,
				hf_approx = False, clus = True, do_plot = False):
		'''
...
#				print(self._grp_lst[g])
#				if g==0:
#					for j in range(3):
#						print([self.HFvec[self._grp_lst[g][k]][j] for k in range(len(self._grp_lst[g]))])
#						self._over_op.append(sum(self.HFvec[self._grp_lst[g][k]][j]*self.In_tens_disjoint[g][k][j] for k in range(len(self._grp_lst[g]))))
#				else:
#					for j in range(3):
#						print(j)
#						self._over_op[j]+=sum(self.HFvec[self._grp_lst[g][k]][j]*self.In_tens_disjoint[g][k][j] for k in range(len(self._grp_lst[g])))
