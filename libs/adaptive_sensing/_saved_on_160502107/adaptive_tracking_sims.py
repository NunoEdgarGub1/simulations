...
plot_ratio = 0
test_B_statistics = 0
folder = r'D:/Research/WorkData/adptv_tracking_sim/OH_0.01us/dB=50_sweep_N/'

def compare (stamp, sweep_par='dB', fom_thr=0, fill_gaps=False, do_save = False, filename = None):

	d_array = [stamp]
	a = trackSeq.Analyze(folder = folder)
	a.set_analysis_params (fom_thr = fom_thr, sigma_thr = 0, fill_gaps = fill_gaps)
	if (sweep_par == 'dB'):
...
def analyze (stamp, nr_reps = 3):
	...
	s.analysis (reps = 'all', fom_thr = 0, sigma_thr = 0, fill_gaps = True)
